...
// 1. Initialize ALL form data variables with empty values
//    'alamat_surat' and '$alamat_suratErr' removed
$nama_pelajar = $no_pelajar = $no_kad_pengenalan = $kampus = $kod_program = $no_telefon = $semester = "";
$mod_pengajian_semasa = ""; // For radio button
$taraf_pengajian_semasa = []; // For checkboxes, initialize as empty array

$program_dipohon = $mod_pengajian_baru = $kampus_baru = $sesi_pertukaran = $alasan_permohonan = "";

// 2. Initialize ALL error variables with empty values
//    'alamat_suratErr' removed
$nama_pelajarErr = $no_pelajarErr = $no_kad_pengenalanErr = $kampusErr = $kod_programErr = $no_telefonErr = $semesterErr = "";
$mod_pengajian_semasaErr = $taraf_pengajian_semasaErr = "";
$program_dipohonErr = $mod_pengajian_baruErr = $kampus_baruErr = $sesi_pertukaranErr = $alasan_permohonanErr = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  // --- BUTIRAN PELAJAR (STUDENT DETAILS) VALIDATION ---

  // Nama Pelajar
  if (empty($_POST["nama_pelajar"])) {
    $nama_pelajarErr = "Nama Pelajar diperlukan.";
    $form_valid_php = false;
  } else {
    $nama_pelajar = htmlspecialchars($_POST["nama_pelajar"]);
    // Optional: Add regex for name if needed (e.g., only letters and spaces)
    if (!preg_match("/^[a-zA-Z\s.-]+$/", $nama_pelajar)) {
        $nama_pelajarErr = "Nama hanya boleh mengandungi huruf, ruang, titik atau sengkang.";
        $form_valid_php = false;
    }
  }

  // No. Pelajar
  if (empty($_POST["no_pelajar"])) {
    $no_pelajarErr = "No. Pelajar diperlukan.";
    $form_valid_php = false;
  } else {
    $no_pelajar = htmlspecialchars($_POST["no_pelajar"]);
    // Optional: Add regex for student ID if specific format (e.g., only digits)
    if (!preg_match("/^\d+$/", $no_pelajar)) {
        $no_pelajarErr = "No. Pelajar mesti nombor.";
        $form_valid_php = false;
    }
  }

  // No. Kad Pengenalan
  if (empty($_POST["no_kad_pengenalan"])) {
    $no_kad_pengenalanErr = "No. Kad Pengenalan diperlukan.";
    $form_valid_php = false;
  } else {
    $no_kad_pengenalan = htmlspecialchars($_POST["no_kad_pengenalan"]);
    // Must be exactly 12 digits
    if (!preg_match("/^\d{12}$/", $no_kad_pengenalan)) {
        $no_kad_pengenalanErr = "No. Kad Pengenalan mestilah 12 digit.";
        $form_valid_php = false;
    }
  }
  ...
  if (empty($_POST["mod_pengajian_semasa"])) {
    $mod_pengajian_semasaErr = "Mod Pengajian Semasa diperlukan.";
    $form_valid_php = false;
  } else {
    $mod_pengajian_semasa = htmlspecialchars($_POST["mod_pengajian_semasa"]);
  }
  ...
  // --- PERMOHONAN PERTUKARAN (APPLICATION REASON) VALIDATION ---
  ...
  if ($form_valid_php) {
    ...
    $_SESSION['form_data_pertukaran'] = [
        ...
        'semester' => $semester,
        'mod_pengajian_semasa' => $mod_pengajian_semasa,
    ...
    header("Location: submission.php"); // Redirect to welcome.php
    exit();
  }
}
?>
